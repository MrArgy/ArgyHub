...
colorama.init()
print(Fore.RED + Style.BRIGHT + ">>> по вопросам к @MrArgy или к @ArgyDevBOT")
print("")
print(Fore.BLUE + Style.BRIGHT + ">> ввод своих данных:")
print(Fore.WHITE + Style.RESET_ALL + "1. введите свой номер телефона")
print("2. напишите Y для подтверждения номера")
print("3. введите код который придёт в тг")
print("4. пароль от двухэтапной авторизации (если он есть) ")
print("(если вы уже зареганы то просто подождите)")
print(Fore.YELLOW + "")
...
async def kviks(client, message):
	...
	kviks = [f'Квикс согласен 😁',f'Квикс не согласен 😲',f'Квикс не будет отвечать на это ☹',f'Квикс разозлился и отвечать на это не будет 😡',f'Квикс засмеялся 🤣',f'Квикс с хорошим настроением сказал что он согласен 😀 (РЕДКИЙ ОТВЕТ)']
	text = f"Квикс ты {random.choice(vopross)}"
	await message.edit(str(text))
	await asyncio.sleep(3.0)
...
async def bombachat(client, message):
	global number
	number = number + 1
	pryatka = ['Все участники выжили... Повезло. Отпразнуем этот день 😎', 'Никто не выжил... Сегодня не наш день 😔', 'Все выжили кроме одного 😨. Как такое возможно?', 'Никто не выжил кроме одного... Он наверное обыграл русское казино']
	...
	await asyncio.sleep(3.0)
	text13 = random.choice(pryatka)
	await message.edit(str(text13))
	await asyncio.sleep(5.0)
...
def valentine(app, message):
	...
	скрипт создан челом: @MrArgy\nв создании участвовал еще один человек\nсписок команд: скоро
	
	купить ultimate версию за 5 гривен (с командами админа и прочей всякой хуйней): скоро
...
def mems(app, msg):
	...
	(Примичание: Они могут подгружатся дольше чем надо, смотря какая скорость у интернета.)
	 ...
	 2) Команда: ".блядьнахуй"
	 3) Команда: ".гдетыблядь"
	 4) Команда: ".даунобосаный"
	 5) Команда: ".ктокуда"
	 6) Команда: ".уменяестьплан"
	 7) Команда: ".ятрахнутебя"

	
	(Все команды нужно писать без ковычек)
	Автор скрипта: @MrArgy
		''')
	global number
	number = number + 1
